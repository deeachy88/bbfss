...
from certification.forms import GapForm, FpcForm, OCForm
...
def save_organic_certificate(request):
    ...
    if Applicant_Type == 'I':
        ...
        cid = None
        Name = None
        Farmer_Group_name = request.POST.get('farmers_group_name')
        Farmer_Group_No = request.POST.get('farmers_group_number')
...
def save_aquaculture_details(request):
    ...
    aqua_Sold_unit = request.POST.get('aqua_Sold_unit')
...
def add_pack_house_details(request):
    application_no = request.GET.get('pack_house_application_no')
    ...
    t_certification_gap_t5.objects.create(
        Application_No=application_no,
...
def gap_farm_input_observation(request):
    ...
    t_certification_gap_t7.objects.create(Application_No=application_no,
                                          ...
                                          Audit_Findings_Farm_Supplier=Audit_Findings_Farm_Supplier,
                                          Audit_Findings_Farm_Invoice=Audit_Findings_Farm_Invoice)
    details = t_certification_gap_t7.objects.filter(Application_No=application_no)
...
def approve_gap_application(request):
    ...
    details.update(Validity_Period=validity)
    details.update(Approve_Date=date.today())
    details.update(Certificate_Type=certificate_type)
    if remarks is not None:
        details.update(FO_Remarks=remarks)
    else:
        details.update(FO_Remarks=None)
...
def gap_certificate_no(request):
    ...
    if not lastAppNo:
        year = timezone.now().year
        newAppNo = "BGCL" + "-" + str(year) + "-" + "0001"
    else:
        substring = str(lastAppNo)[10:13]
        substring = int(substring) + 1
        AppNo = str(substring).zfill(4)
        year = timezone.now().year
        newAppNo = "BGCL" + "-" + str(year) + "-" + AppNo
    return newAppNo
...
def save_food_product_certificate(request):
    ...
    date_from_p = request.POST.get('From_Date')
    date_to_p = request.POST.get('P_To_Date')
    ...
    date_from_c = request.POST.get('C_From_Date')
    date_to_c = request.POST.get('C_To_Date')
    ...
    if C_Export_From_Date:
        date_export_f = C_Export_From_Date
    else:
        date_export_f = None
    if C_Export_To_Date:
        date_export_t = C_Export_To_Date
    else:
        date_export_t = None
    if P_Export_From_Date:
        date_export_pf = P_Export_From_Date
    else:
        date_export_pf = None
    if P_Export_To_Date:
        date_export_pt = P_Export_To_Date
    else:
        date_export_pt = None
...
def delete_farm_details(request):
    ...
    if identification_type == 'GAP':
        ...
        return render(request, 'GAP_Certification/farm_input_details.html', {'farm_inputs': farm_inputs})
    elif identification_type == 'FPC':
        details = t_certification_food_t3.objects.filter(Record_Id=record_id)
        details.delete()
...
def delete_farmers_group(request):
    ...
    if identification_type == 'GAP':
        ...
        details.delete()
        farmer_group = t_certification_gap_t2.objects.filter(Application_No=application_no).order_by('Record_Id')
        return render(request, 'GAP_Certification/farmer_group_details.html', {'farmers_group_details': farmer_group})
    else:
        details = t_certification_organic_t2.objects.filter(Record_Id=record_id)
        details.delete()
        farmer_group = t_certification_organic_t2.objects.filter(Application_No=application_no).order_by('Record_Id')
        return render(request, 'organic_certification/farmer_group_details.html',
                      {'farmers_group_details': farmer_group})
...
def organic_gewog(request):
    ...
    return render(request, 'organic_certification/gewog_list.html', {'gewog_list': gewog_list})


def organic_village(request):
    ...
    return render(request, 'organic_certification/village_list.html', {'village_list': village_list})
